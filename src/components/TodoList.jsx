...
import { toggleComplete, setEditTodo, setShowEditAddModal, setShowDeleteConfirmModal, setTodoToDelete } from '../redux/todosSlice';
...
const TodoList = () => {
  ...
  const handleDelete = (todoId) => {
    dispatch(setTodoToDelete(todoId));
        ...
        todos.map((todo) => (
          <div key={todo.id} className={`todo-item ${todo.color} d-flex align-items-start p-3 border rounded mb-2`}>
            ...
            <div className="button-group d-flex align-items-center ml-3">
              ...
              ></div>
              <PencilSquare className="edit-button m-2 fs-4" onClick={() => handleEdit(todo)} style={{ cursor: 'pointer' }} />
              <TrashFill className="edit-button text-danger fs-4" onClick={() => handleDelete(todo.id)} style={{ cursor: 'pointer' }} />
            </div>
          </div>
        ))
      )}
